...
def handle(request, context):
    ...
    distance_mm_2 = request.get("distance_mm_2")
    ...
    try:
        ...
        return {"ok": False, "error": "distance_mm must be numeric"}
    ...
    if distance_mm_2 is not None:
        try:
            distance_mm_2 = float(distance_mm_2)
        except Exception:
            return {"ok": False, "error": "distance_mm_2 must be numeric"}
        if distance_mm_2 <= 0:
            return {"ok": False, "error": "distance_mm_2 must be > 0"}
        if distance_mm_2 > MAX_DISTANCE_MM:
            return {
                "ok": False,
                "error": f"distance_mm_2 exceeds MAX_DISTANCE_MM ({MAX_DISTANCE_MM})",
            }
    ...
    try:
        ...
        dist1 = adsk.core.ValueInput.createByReal(_mm_to_internal(units_mgr, distance_mm))
        if distance_mm_2 is not None:
            dist2 = adsk.core.ValueInput.createByReal(_mm_to_internal(units_mgr, distance_mm_2))
            try:
                chamfer_input.setTwoDistances(dist1, dist2)
            except Exception:
                try:
                    chamfer_input.setToTwoDistances(dist1, dist2)
                except Exception:
                    chamfer_input.distanceOne = dist1
                    chamfer_input.distanceTwo = dist2
        else:
            try:
                chamfer_input.setToEqualDistance(dist1)
            except Exception:
                try:
                    chamfer_input.setToDistance(dist1)
                except Exception:
                    chamfer_input.distance = dist1
        ...
        if feature:
            try:
                ...
                pass
        return {"ok": False, "error": f"Chamfer failed: {exc}", "data": {"preview": {"plan": plan}}}
